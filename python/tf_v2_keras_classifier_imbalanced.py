...
"""
Machine learning algorithms: TensorFlow Keras Binary Classification Imbalanced Data [using TensorFlow 2.x-Keras]
"""
import numpy as np
import pandas as pd
...
# TensorFlow: Create tf dataset
def make_ds(features, label):
    '''
    Create tensorflow dataset (input function for the model) from pandas df
    :param features: pandas dataframe
    :param label: pandas dataframe
    :return: ds: tensorflow dataset
    '''
    # Construct a dataset
    ds = tf.data.Dataset.from_tensor_slices((dict(features), label))
    shuffle_buffer_size = 100000  # to perfect shuffling, value greater than full data size
    ds = ds.shuffle(shuffle_buffer_size).repeat(None)  # None: dataset be repeated indefinitely
    return ds
...
if __name__ == '__main__':
    ...
    # TensorFlow: Create train dataset (oversampling) [0:major class, 1: minor class]
    neg, pos = np.bincount(y_train)  # count number of occurrences of each label
    # initial_bias = np.log([pos/neg]) # set initial output layer's bias for more reasonable initial guesses
    # initial_weight = {0:1/neg, 1:1/pos} # heavily weight the few examples (1's label) that are available

    # TensorFlow: Oversample the minority class using tf.data
    X_train_1 = X_train[y_train == 1]
    X_train_0 = X_train[y_train == 0]
    y_train_1 = y_train[y_train == 1]
    y_train_0 = y_train[y_train == 0]

    # TensorFlow: Create tf dataset from pandas df
    ds_1 = make_ds(X_train_1, y_train_1)
    ds_0 = make_ds(X_train_0, y_train_0)

    # TensorFlow: Create resampled dataset
    resampled_ds = tf.data.experimental.sample_from_datasets([ds_1, ds_0], weights=[0.5, 0.5])
    resampled_ds = resampled_ds.batch(batch_size)\
        .prefetch(2)  # allows later elements to be prepared while current is being processed

    resampled_steps_per_epoch = np.ceil(2.0 * neg / batch_size)  # to use all the train data for one epoch

    # TensorFlow: Create validation and test dataset (no need for oversampling)
    ...
    history = model.fit(resampled_ds,  # (train_features, train_label)
                        steps_per_epoch=resampled_steps_per_epoch, # batches of samples for fitting to finish one epoch
    ...
    model.save('tf_keras_classification_imbalanced.model')

    # TensorFlow: Restore the model
    model = tf.keras.models.load_model('tf_keras_classification_imbalanced.model')
    ...
    plt.savefig('tensorflow_keras_classification_imbalanced.png', bbox_inches='tight')
    plt.show()
